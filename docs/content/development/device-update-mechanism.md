...
**Each EEPROM page is attempted up to four times before the transfer is given up.** A firmware image is a little over two thousand 64-byte page writes and a few hundred readback reads, and until this was added every one of them had to succeed or the whole update was abandoned — which is an unreasonable thing to ask of an I2C bus. The SDK documents its own timeout return as retryable with no recovery needed, and a board whose bus is marginal (long wiring, weak pull-ups, a rail sagging under a just-configured FPGA) fails one page somewhere in the middle, in a different place every run, and never completes. Between attempts the firmware settles the bus and then waits for the part to acknowledge its own address again rather than guessing how long its internal write cycle needs.

Retrying is safe because a page write is idempotent and the whole page is rewritten, not the part of it that did not go: a transfer that died part way has left an unknown number of bytes in the part's page buffer, and refilling it is the only way to know what is in there. Nothing about this weakens the guarantee, either — what decides whether an update commits is the readback digest over the medium, never the fact that a write returned success.
...
The same asymmetry explains which failures stall and which do not, and the dividing line is *when the answer is known*, not how serious it is. A refusal that can be reached before the request's data stage is read stalls; one that cannot be reached until afterwards is answered through `UPDATE_STATUS`, because the USB hardware acknowledges a control-OUT transfer as soon as its last byte arrives and a request whose payload has been taken can no longer be refused.

So two kinds of refusal stall. A request whose *shape* is wrong — an `UPDATE_BEGIN` that is not 40 bytes, a chunk larger than the advertised maximum. And **an `0xD2` the transfer's own state cannot accept**: the wrong target, an index out of sequence, a length that is not a whole number of pages or would overrun, or a transfer that is no longer receiving because it has already failed. All of that is in the request's fields, so none of it has to wait for the data.

That second case is the one that matters in practice. Without it a device that fails a write part way through an image goes on acknowledging every chunk that follows, and a host has no other signal to read: it sends the whole remaining image — tens of seconds of it — into a transfer that ended long ago, and only learns what happened when it reads `UPDATE_STATUS` at the end. Stalling costs no diagnosis, because the host reads `UPDATE_STATUS` on a failed chunk anyway and the *first* error recorded is the one kept: what it finds there is the write that failed and the offset it reached, not the sequence refusal that followed from it.

A content refusal still cannot stall, and `UPDATE_FINISH` still does not: "the update failed and here is which check caught it" is worth more to whoever is looking at the screen than an endpoint that simply stopped answering.
...
- the firmware's **protocol version**, carried in the USB descriptor's `bcdDevice` field. That field was a dead `0x0000` until this work, and it is the ideal place for it: the host reads it during enumeration, before opening the device and without sending a single vendor request, so a device speaking a protocol this application does not understand can be recognised before anything is asked of it.
    ...
    The version is the **high byte** and the low byte is zero, so version 1 is `0x0100` and `lsusb` reads it as `1.00` rather than as something that looks like a mistake. The host compares the high byte, and only for a device running this firmware: the Cypress boot ROM and the flash programmer personality put their own numbering in that field, and reading one of those as a protocol version would have the application deciding what a device supports from a number that means something else.
